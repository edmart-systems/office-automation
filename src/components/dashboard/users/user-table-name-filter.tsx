...
const UserTableNameFilter = () => {
  ...
  const setSearchParams = (searchName: string) => {
    ...
    if (currentParams.has("name")) {
      ...
      currentParams.append("name", searchName);
    } else {
      currentParams.append("name", searchName);
    }
    nProgress.start();
  ...
  return (
    <PopupState variant="popover" popupId={`user-name-popup-menu`}>
      {(popupState) => (
        <React.Fragment>
          <Button
            variant="contained"
          ...
          <Menu {...bindMenu(popupState)}>
            <form onSubmit={(evt) => handleSubmit(evt, popupState)}>
              <MyMenuItem>
                <Typography>Filter by User name</Typography>
              </MyMenuItem>
              <MyMenuItem>
                <TextField
                  size="small"
...
export default UserTableNameFilter;
